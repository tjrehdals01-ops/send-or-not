...
├─ lib/
│  └─ message.ts        # 채널·언어·말투별 생성 및 분석 로직
├─ public/
...
개발 서버가 안내하는 로컬 주소를 브라우저에서 열면 됩니다.
...
현재 공개 버전은 발표와 사용자 흐름 검증을 위한 **클라이언트 기반 프로토타입**입니다. 키워드, 채널, 언어, 목적에 따라 분석 기준과 문장 템플릿을 적용하므로 입력 내용이 외부 서버에 저장되지 않습니다.

실제 서비스로 확장할 경우에는 다음 기능을 추가할 수 있습니다.

- LLM API를 활용한 더 세밀한 맥락 해석
...
이 프로젝트는 입력한 메시지를 데이터베이스에 저장하지 않습니다. 실제 개인정보나 민감한 내용을 입력할 때에는 사용자가 최종 문장을 직접 확인해야 합니다.
